...
class CourseReportPage2:
    def __init__(self,hwindow):
        ...
        self.window.title('Course detail by department ')
        ...
        self.headlbl = Label(self.window,text="Course Report By Department",font=("Georgia",35,"bold"),
                             background=mycolor3,foreground=mycolor1,relief="groove",borderwidth=10)

        self.l1=Label(self.window,text="Select Department",font=myfont1,background=mycolor3,foreground=mycolor1)
        self.v1 = StringVar()
        ...
        # ------------------- table ------------------------------------
        self.mytable = Treeview(self.window,columns=['c1','c2','c3','c4','c5'],height=20)
    ...
    def getPrintout(self):
        pdf = my_cust_PDF()
        headings = ['Course', 'Department', 'Duration', 'Units', 'Fee']
    ...
    def getAllData(self):
        try:
            ...
            qry = "select * from course where department_name=%s"
            rowcount = self.curr.execute(qry, (self.v1.get()))
            data = self.curr.fetchall()
            self.pdata=[]
            if data:
                for myrow in data:
                    r1 = [myrow[0],myrow[1],myrow[2],myrow[3],myrow[4]]
...
if __name__ == '__main__':
    dummyhomepage=Tk()
    CourseReportPage2(dummyhomepage)
